...
void VulkanEngine::init_imgui() {
  ...
  ImFont *jetbrainsMono =
      io.Fonts->AddFontFromFileTTF("../fonts/jetbrains_mono.ttf", 18);
  io.Fonts->Build();
...
void VulkanEngine::draw() {
  ...
  //	if (result_present == VK_ERROR_OUT_OF_DATE_KHR || result_present ==
  // VK_SUBOPTIMAL_KHR)
  //	{
  //		_wasResized = true;
  //		return;
...
void VulkanEngine::init_default_renderpass() {
  ...
  color_attachment_ref.layout =
      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // Image is on a layout optimal
...
void VulkanEngine::init_pipelines() {
  ...
  pushConstantRange.stageFlags =
      VK_SHADER_STAGE_VERTEX_BIT; // because the mesh shader is a vertex shader

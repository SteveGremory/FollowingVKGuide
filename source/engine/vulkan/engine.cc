...
void VulkanEngine::init() {
        ...
        SDL_WindowFlags window_flags =
            (SDL_WindowFlags)(SDL_WINDOW_VULKAN | SDL_WINDOW_SHOWN |
                              SDL_WINDOW_RESIZABLE);

        _window = SDL_CreateWindow("Vulkan Engine", SDL_WINDOWPOS_CENTERED,
                                   SDL_WINDOWPOS_CENTERED, _windowExtent.width,
...
void VulkanEngine::draw() {
        ...
        // make a clear-color from frame number. This will flash with a 120*pi
        // frame period.
        ...
        // wait for the semaphore to say, "alright mate come in, Dr. swapchain
        // is ready to check yo ass" wait for the second semaphore to say,
        // "alright mate, the wait is over; Dr. rendering has finished the
        // surgery"
        ...
        // renderfence will now block everything until this shit finishes
        // shitting
        ...
        // as it's necessary that drawing commands have finished before the
...
void VulkanEngine::run() {
        ...
        while (!bQuit) {
                ...
                while (SDL_PollEvent(&e) != 0) {
                        ...
                        // close the window when user alt-f4s or clicks the X
                        // button
                        if (e.type == SDL_QUIT) {
                                bQuit = true;
                        }
...
void VulkanEngine::init_vulkan() {
        ...
        auto inst = builder.set_app_name("")
                        .request_validation_layers(true)
                        .require_api_version(1, 2, 0)
                        .use_default_debug_messenger();

        auto inst_ret = inst.build();
        ...
        VkPhysicalDeviceVulkan11Features feat{};
...
VkSampleCountFlagBits VulkanEngine::get_max_usable_sample_count() {
        VkSampleCountFlags counts =
            _deviceProperties.limits.framebufferColorSampleCounts &
        ...
        std::cout << "Max Sample Count Available: VK_SAMPLE_COUNT_1_BIT"
                  << std::endl;
...
void VulkanEngine::init_swapchain(bool setOld) {
        ...
        VkExtent3D resolveImageExtent = {_windowExtent.width,
                                         _windowExtent.height, 1};
        ...
        depth_buffer_allocation_info.requiredFlags =
        ...
        VkImageCreateInfo resolve_info =
            vkinit::create_image_info(_swapchain_image_format,
        ...
        resolve_allocation_info.requiredFlags =
            VkMemoryPropertyFlags(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        // alloc and create the image
        vmaCreateImage(_allocator, &resolve_info, &resolve_allocation_info,
                       &_resolveImage._image, &_resolveImage._allocation,
                       nullptr);
        // build an image view for the depth buffer to use for rendering
        VkImageViewCreateInfo resolve_view_info =
            vkinit::create_image_view_info(_swapchain_image_format,
                                           _resolveImage._image,
...
void VulkanEngine::init_commands() {
        ...
        -> Once a command buffer has been submitted, it’s still “alive”,
            and being consumed by the GPU, at this point it is NOT safe to reset
        the command buffer yet. You need to make sure that the GPU has finished
        executing all of the commands from that command buffer until you can
        reset and reuse it.
      ...
      */
        ...
        for (int i = 0; i < FRAME_OVERLAP; i++) {
                ...
                _mainDeletionQueue.push_function([=]() {
                        vkDestroyCommandPool(_device, _frames[i]._commandPool,
                                             nullptr);
                });
        }
        ...
        _mainDeletionQueue.push_function([=]() {
                vkDestroyCommandPool(_device, _uploadContext._commandPool,
                                     nullptr);
        });
}
...
void VulkanEngine::init_default_renderpass() {
        ...
        // we don't know or care about the starting layout of the image
        // attachment
        color_attachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        // we use the KHR layout (Image is on a layout that allows displaying
        // the image on the screen)
        color_attachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
        // Now that our main image target is defined, we need to add a subpass
        // that will render into it.
        VkAttachmentReference color_attachment_ref{};
        color_attachment_ref.attachment = 0;
        color_attachment_ref.layout =
            VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // Image is on a layout
                                                      // optimal to be written
                                                      // into by rendering
                                                      // commands.
        ...
        // array of 3 attachments, one for the color, and other for depth and
        // one for MSAA
...
void VulkanEngine::init_framebuffers() {
        ...
        for (int i = 0; i < swapchain_imagecount; i++) {
                ...
                _swapchainDeletionQueue.push_function([=]() {
                        vkDestroyFramebuffer(_device, _frameBuffers[i],
                                             nullptr);
                        vkDestroyImageView(_device, _swapchain_image_views[i],
                                           nullptr);
                });
        }
}
...
void VulkanEngine::init_sync_structures() {
        ...
        for (int i = 0; i < FRAME_OVERLAP; i++) {
                ...
                VK_CHECK(vkCreateFence(_device, &fenceInfo, nullptr,
                                       &_frames[i]._fence));
                ...
                _mainDeletionQueue.push_function([=]() {
                        vkDestroySemaphore(_device, _frames[i]._renderSemaphore,
                                           nullptr);
                        vkDestroySemaphore(
                            _device, _frames[i]._presentSemaphore, nullptr);
                });
        }
...
void VulkanEngine::init_pipelines() {
        ...
        if (!load_shader_module("../shaders/compiled/tri_mesh.vert.spv",
                                &meshVertShader)) {
                std::cout << "Failed to create Mesh fragment shader."
                          << std::endl;
        } else {
                std::cout << "Successfully created fragment shader."
                          << std::endl;
        }
        ...
        pipelineBuilder._shaderStages.push_back(
            vkinit::pipeline_shader_stage_create_info(
                VK_SHADER_STAGE_FRAGMENT_BIT, colorMeshShader));

        // tells vulkan how to read vertices, isn't in use rn.
        pipelineBuilder._vertexInputInfo =
            vkinit::vertex_input_stage_create_info();
        ...
        pipelineBuilder._inputAssembly = vkinit::input_assembly_create_info(
        ...
        pipelineBuilder._multisampling =
            vkinit::multisampling_state_create_info(_sampleCount);
        ...
        pushConstantRange.stageFlags =
            VK_SHADER_STAGE_VERTEX_BIT; // because the mesh shader is a vertex
                                        // shader
        ...
        VkDescriptorSetLayout setLayouts[2] = {_globalSetLayout,
                                               _objectSetLayout};
...
void VulkanEngine::load_meshes() {
        ...
        _carMesh.load_from_obj("../models/suzanne.obj");
...
void VulkanEngine::upload_mesh(Mesh& mesh) {
        ...
        immediate_submit([=](VkCommandBuffer cmd) {
                ...
                vkCmdCopyBuffer(cmd, stagingBuffer._buffer,
                                mesh._vertexBuffer._buffer, 1, &copy);
        });
...
void VulkanEngine::init_scene() {
        ...
        // apparently we create a lotta triangles in a grid and place them
        // around the monkee idfk how
        for (int x = -20; x <= 20; x++) {
                for (int y = -20; y <= 20; y++) {
                        ...
                        // well now i do, so the x and y are positions for the
                        // triangles and there are currently 80 triangles here
                        // aka 80 drawcalls + that 1 for the mesh (it's unified)
                        glm::mat4 translation =
                            glm::translate(glm::mat4{1.0f}, glm::vec3{x, 0, y});
                        glm::mat4 scale = glm::scale(
                            glm::mat4{1.0f}, glm::vec3(0.2f, 0.2f, 0.2f));

                        triangles.transformMatrix = translation * scale;

                        // illuminati moment + triangle moment + didn't ask +
                        // who asked ... and so on
...
void VulkanEngine::draw_objects(VkCommandBuffer cmd, RenderObject* first,
        ...
        glm::mat4 projection = glm::perspective(
            45.0f, (float)_windowExtent.width / (float)_windowExtent.height,
            0.1f, 200.0f);
        ...
        vmaUnmapMemory(_allocator,
                       get_current_frame().cameraBuffer._allocation);
        ...
        char* sceneData;
        vmaMapMemory(_allocator, _sceneParamsBuffer._allocation,
                     (void**)&sceneData);
        ...
        vmaUnmapMemory(_allocator,
                       get_current_frame().objectBuffer._allocation);

        Mesh* lastMesh = nullptr;
        Material* lastMaterial = nullptr;

        /*
        There is no need to rebind the same vertex buffer over and over between
        draws, and the pipeline is the same, but we are pushing the constants on
        every single call. The loop here is a lot higher performance that you
        would think. This simple loop will render thousands and thousands of
        objects with no issue. Binding pipeline is a expensive call, but drawing
        the same object over and over with different push constants is very
        fast.
            - VkGuide.
      */
        ...
        for (int i = 0; i < count; i++) {
                ...
                // only bind the pipeline if it doesnt match with the already
                // bound one
                if (object.material != lastMaterial) {
                        ...
                        uint32_t uniform_offset =
                            pad_uniform_buffer(sizeof(GPUSceneData)) *
                            frameIndex;
                        vkCmdBindDescriptorSets(
                            cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            object.material->pipelineLayout, 0, 1,
                            &get_current_frame().globalDescriptorSet, 1,
                            &uniform_offset);
                        ...
                        vkCmdBindDescriptorSets(
                            ...
                            &get_current_frame().objectDescriptorSet, 0,
                            nullptr);
                }
                ...
                if (object.mesh != lastMesh) {
                        ...
                        vkCmdBindVertexBuffers(
                            cmd, 0, 1, &object.mesh->_vertexBuffer._buffer,
                            &offset);
...
FrameData& VulkanEngine::get_current_frame() {
        ...
        // This will be very useful here. With a frame overlap of 2 (the
        // default), it means that even frames will use _frames[0], while odd
        // frames will use _frames[1].
...
void VulkanEngine::init_descriptors() {
        ...
        vkCreateDescriptorPool(_device, &descPoolInfo, nullptr,
                               &_descriptorPool);
        ...
        vkCreateDescriptorSetLayout(_device, &setinfo, nullptr,
                                    &_globalSetLayout);
        ...
        vkCreateDescriptorSetLayout(_device, &setinfo2, nullptr,
                                    &_objectSetLayout);

        const size_t sceneParamBufferSize =
            FRAME_OVERLAP * pad_uniform_buffer(sizeof(GPUSceneData));
        _sceneParamsBuffer = create_buffer(sceneParamBufferSize,
                                           VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                           VMA_MEMORY_USAGE_CPU_TO_GPU);

        for (int i = 0; i < FRAME_OVERLAP; i++) {
                _frames[i].cameraBuffer = create_buffer(
                    sizeof(GPUCameraData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                    VMA_MEMORY_USAGE_CPU_TO_GPU);

                const int MAX_OBJECTS = 10000;
                _frames[i].objectBuffer =
                    create_buffer(sizeof(GPUObjectData) * MAX_OBJECTS,
                ...
                VkWriteDescriptorSet objectWrite =
                    vkinit::write_descriptor_buffer(
                        ...
                        _frames[i].objectDescriptorSet, &objectInfo, 0);
        ...
        _mainDeletionQueue.push_function([&]() {
                ...
                vkDestroyDescriptorSetLayout(_device, _objectSetLayout,
                                             nullptr);
                vkDestroyDescriptorSetLayout(_device, _globalSetLayout,
                                             nullptr);
                ...
                for (int i = 0; i < FRAME_OVERLAP; i++) {
                        ...
                        vmaDestroyBuffer(_allocator,
                                         _frames[i].objectBuffer._buffer,
...
void VulkanEngine::immediate_submit(
    std::function<void(VkCommandBuffer)>&& function) {
        ...
        VkCommandBufferAllocateInfo immediateCommandBufferInfo =
            vkinit::command_buffer_allocate_info(_uploadContext._commandPool);

        // Allocate the command buffer
        VkCommandBuffer cmd;
        VK_CHECK(vkAllocateCommandBuffers(_device, &immediateCommandBufferInfo,
                                          &cmd));

...
void VulkanEngine::draw() {
    ...
    // ready to check yo ass" wait for the second semaphore to say, "alright
    // mate, the wait is over; Dr. rendering has finished the surgery"
...
void VulkanEngine::run() {
    ...
    while (!bQuit) {
        ...
        deltaTime =
            (double)((NOW - LAST) * 10 / (double)SDL_GetPerformanceFrequency());
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            if (e.type == SDL_WINDOWEVENT) {
                if (e.window.event == SDL_WINDOWEVENT_SIZE_CHANGED &&
                    ...
                    if (_windowExtent.height != e.window.data2 ||
...
void VulkanEngine::init_swapchain(bool setOld) {
    ...
    VkImageViewCreateInfo resolve_view_info = vkinit::create_image_view_info(
        _swapchain_image_format, _resolveImage._image,
...
void VulkanEngine::init_commands() {
    ...
    command buffer yet. You need to make sure that the GPU has finished
    executing all of the commands from that command buffer until you can reset
    and reuse it.
...
void VulkanEngine::init_default_renderpass() {
    ...
    color_attachment_ref.layout =
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // Image is on a layout
                                                  // optimal to be written into
                                                  // by rendering commands.
    ...
    // array of 3 attachments, one for the color, and other for depth and one
    // for MSAA
...
void VulkanEngine::init_sync_structures() {
    ...
    for (int i = 0; i < FRAME_OVERLAP; i++) {
        ...
        VK_CHECK(
            vkCreateFence(_device, &fenceInfo, nullptr, &_frames[i]._fence));
...
void VulkanEngine::load_meshes() {
    ...
    _carMesh.load_from_obj("../models/low_DEF.obj");
...
void VulkanEngine::init_scene() {
    ...
    // apparently we create a lotta triangles in a grid and place them around
    // the monkee idfk how
    for (int x = -20; x <= 20; x++) {
        for (int y = -20; y <= 20; y++) {
            ...
            // illuminati moment + triangle moment + didn't ask + who asked ...
            // and so on
...
void VulkanEngine::draw_objects(VkCommandBuffer cmd, RenderObject* first,
    ...
    every single call. The loop here is a lot higher performance that you would
    think. This simple loop will render thousands and thousands of objects with
    no issue. Binding pipeline is a expensive call, but drawing the same object
    over and over with different push constants is very fast.
        - VkGuide.
  */
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (object.material != lastMaterial) {
            ...
            uint32_t uniform_offset =
                pad_uniform_buffer(sizeof(GPUSceneData)) * frameIndex;
        ...
        if (object.mesh != lastMesh) {
            ...
            vkCmdBindVertexBuffers(
                cmd, 0, 1, &object.mesh->_vertexBuffer._buffer, &offset);
